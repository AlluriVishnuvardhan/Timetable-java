...
// New function to handle Excel file uploads
async function handleExcelUpload(input) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Parse the data into the 'faculties' array
            faculties = parseExcelData(rawData);
            render();
            document.getElementById('manual-tab').click(); // Switch to the manual input tab
        } catch (error) {
...
// Helper function to parse raw Excel data into the desired format
function parseExcelData(rawData) {
    const data = rawData.slice(1); // Skip header row
    ...
    data.forEach(row => {
        ...
        if (rowData.Name) {
            if (currentFaculty) {
                newFaculties.push(currentFaculty);
            }
            currentFaculty = {
                Name: rowData.Name,
                Full_Name: rowData.Full_Name || rowData.Name,
                Designation: rowData.Designation || "Professor",
                Shift: String(rowData.Shift) || "8-3",
                Weekly_Hours: parseInt(rowData.Weekly_Hours) || 18,
                Subjects: []
            };
        }

        if (currentFaculty && rowData.Subject) {
            const subjectEntry = {
                Type: rowData.Type,
                Semester: String(rowData.Semester),
                Division: String(rowData.Division),
                Div_Shift: String(rowData.Div_Shift) || "8-3",
                Subject: String(rowData.Subject),
                Course_Code: String(rowData.Course_Code || ""),
                Holidays: rowData.Holidays ? String(rowData.Holidays).split(',').map(h => h.trim()) : [],
                Num_Holidays: parseInt(rowData.Num_Holidays) || 0
            };

            if (rowData.Type === 'Theory') {
                subjectEntry.Theory_Classes = parseInt(rowData.Theory_Classes) || 1;
            } else { // Lab
                subjectEntry.Num_Labs = parseInt(rowData.Num_Labs) || 1;
                subjectEntry.Batches = rowData.Batches ? String(rowData.Batches).split(',').map(b => b.trim()) : ["B1"];
                subjectEntry.Batches_Grouped = String(rowData.Batches_Grouped).toLowerCase() === 'true';
            }
            currentFaculty.Subjects.push(subjectEntry);
        }
    });
...
// Updated submitAll function with more robust error handling
async function submitAll(type) {
    ...
    try {
        if (type === 'manual') {
            ...
            if (!data.ok) {
                const errorMessage = (data.error && typeof data.error === 'string') ? data.error : "Generation failed. Please check the server logs for details.";
        ...
        errorBox.textContent = err.message || "An unknown error occurred.";
